...
class SequentialTrainer_(TrainerBase):
    def __init__(self, args):
        super().__init__(args)
    ...
    def init_dataset(self, dataset_config):
        ...
        meshgrid = np.meshgrid(np.linspace(x_min, x_max, dataset_config["latent_queries"][0]),
                            np.linspace(y_min, y_max, dataset_config["latent_queries"][1]),
                            indexing='ij')
        regional_points = np.stack(meshgrid, axis=-1)  # [\sqrt{num_queries}, \sqrt{num_queries}, 2]
        self.regional_points = torch.tensor(regional_points, dtype=self.dtype).unsqueeze(0).to(self.device)  # [1, \sqrt{num_queries}, \sqrt{num_queries}, 2]
    ...
    def init_model(self, model_config):
        ...
        self.rigraph = RegionInteractionGraph.from_point_cloud(self.x_train[0][0],
                                                    ...
                                                    regional_points=self.regional_points.reshape(-1,2),
                                                    )
