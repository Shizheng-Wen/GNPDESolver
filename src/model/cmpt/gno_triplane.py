...
class GNOEncoder(nn.Module):
    def __init__(self, in_channels, out_channels, gno_config):
        ...
        if gno_config.node_embedding:
            ...
            self.coord_dim = gno_config.gno_coord_dim * 4 * 2
    ...
    def forward(self, input_coords, queries, pndata, axis_key) -> torch.Tensor:
        batch_size = pndata.shape[0]
        device = pndata.device
        if self.graph_cache.get(axis_key) is None:
            spatial_nbrs_scales = []
            for scale in self.scales:
                ...
                spatial_nbrs_scales.append(spatial_nbrs)
            if self.use_graph_cache:
                self.graph_cache[axis_key] = spatial_nbrs_scales
        else:
            spatial_nbrs_scales = self.graph_cache[axis_key] 
...
class GNODecoder(nn.Module):
    ...
    def forward(self, input_coords, queries, rndata, axis_key) -> torch.Tensor:
