...
class GNOEncoder(nn.Module):
    def __init__(self, in_channels, out_channels, gno_config):
        super().__init__()
        self.nb_search = NeighborSearch_batch(gno_config.gno_use_open3d)
        ...
        self.gno = IntegralTransformBatch(
    ...
    def forward(self, x, latent_queries, pndata):
        ...
        """
        # x = torch.rand(8, 9225, 2).to("cuda:0")
        # latent_queries = torch.rand(8, 4096, 2).to("cuda:0")
        # x = rescale(x)
